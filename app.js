    ...
    // Red de seguridad: re-aplicar el tema guardado con la lógica autoritativa de
    // aplicarTema (fuente única). Así el tema queda bien aplicado aunque el init
    // rápido del <head> fuera de una versión previa/incompleta.
    try { window.aplicarTema(_temaActual()); } catch (e) {}
    ...
    function _notaFaltantesHTML(faltantes, dark) {
        ...
        // Variante clara con ámbar MARCADO: así resalta también sobre fondos
        // pastel (rosa, menta, durazno, aqua…) donde un crema pálido se perdía.
        const bg = dark ? 'rgba(245,158,11,0.12)' : '#fef3c7';
        const bd = dark ? 'rgba(245,158,11,0.35)' : '#f59e0b';
        const tx = dark ? '#fcd34d' : '#7c2d12';
        const chipBg = dark ? 'rgba(245,158,11,0.18)' : '#fde68a';
        const sombra = dark ? 'none' : '0 1px 6px rgba(120,80,0,0.14)';
        const chips = faltantes.map(k => {
            ...
            // Cada día es tocable: abre el ingreso de recaudación con esa fecha.
            return `<button type="button" onclick="abrirModalRec('${k}')" style="display:inline-flex;align-items:center;gap:4px;background:${chipBg};color:${tx};font-size:11px;font-weight:700;padding:4px 10px;border-radius:20px;margin:4px 4px 0 0;white-space:nowrap;border:1px solid ${bd};cursor:pointer;">
                <span class="material-symbols-outlined" style="font-size:13px;">add</span>${txt}
            </button>`;
        }).join('');
    ...
    async function refresh(isFirstLoad=false) {
        ...
        try {
            ...
            (function () {
                ...
                if (cont) {
                    cont.innerHTML = _notaFaltantesHTML(_diasFaltantes, _avisoOscuro) + (fechasOrdenadas.length ? cards : '<div style="text-align:center;padding:32px;color:#c3c7cb;font-size:13px;">Sin movimientos en este periodo</div>');
    ...
    window.abrirModalRec = (fecha) => {
        if (!currentUser) return;
        // Fecha opcional (p. ej. al tocar un día faltante); por defecto, hoy.
        const _f = (typeof fecha === 'string' && /^\d{4}-\d{2}-\d{2}$/.test(fecha))
            ? fecha : new Date().toISOString().split('T')[0];
        document.getElementById('recFecha').value = _f;
    ...
    // Se muestran agrupadas (billetes / monedas) para que la lista se lea de corrido.
    const EGR_GRUPOS = [
        { titulo: 'Billetes', icono: '💵', denoms: [20000, 10000, 5000, 2000, 1000] },
        { titulo: 'Monedas',  icono: '🪙', denoms: [500, 100, 50, 10] }
    ];
    const EGR_DENOMS = EGR_GRUPOS.reduce((acc, g) => acc.concat(g.denoms), []);
    const _egrFmt = v => '$' + Number(v).toLocaleString('es-CL');

    function _egrRenderBilletes() {
        const cont = document.getElementById('egresoBilletes');
        if (!cont) return;
        // Tres columnas fijas: valor · cantidad · subtotal. Así cada número
        // queda debajo de su título y se entiende de una mirada.
        const COLS = 'display:grid;grid-template-columns:1fr 58px 90px;align-items:center;gap:10px;padding:7px 12px;';

        const cabecera = '<div style="' + COLS + 'background:#f1f5f9;border-bottom:1.5px solid rgba(148,163,184,0.28);">'
            + '<span style="font-size:0.6rem;font-weight:800;color:#64748b;text-transform:uppercase;letter-spacing:0.07em;">Valor</span>'
            + '<span style="font-size:0.6rem;font-weight:800;color:#64748b;text-transform:uppercase;letter-spacing:0.07em;text-align:center;">Cant.</span>'
            + '<span style="font-size:0.6rem;font-weight:800;color:#64748b;text-transform:uppercase;letter-spacing:0.07em;text-align:right;">Subtotal</span>'
            + '</div>';

        const grupo = g => '<div style="padding:8px 12px 3px;font-size:0.64rem;font-weight:800;color:#64748b;'
            + 'text-transform:uppercase;letter-spacing:0.07em;">' + g.icono + ' ' + g.titulo + '</div>'
            + g.denoms.map(fila).join('');

        function fila(d) {
            return '<div style="' + COLS + '">'
                + '<label for="egrBil-' + d + '" style="font-size:0.88rem;font-weight:700;color:#1e293b;">' + _egrFmt(d) + '</label>'
                + '<input type="tel" inputmode="numeric" id="egrBil-' + d + '" placeholder="0" '
                +   'oninput="_egrTotalBilletes()" onfocus="this.select()" '
                +   'style="width:100%;padding:7px 2px;border:1.5px solid #e2e8f0;border-radius:10px;font-size:0.92rem;'
                +   'font-weight:700;text-align:center;color:#1e293b;background:#f8fafc;box-sizing:border-box;">'
                + '<span id="egrSub-' + d + '" style="font-size:0.82rem;font-weight:700;text-align:right;color:#94a3b8;">—</span>'
                + '</div>';
        }

        const pie = '<div style="' + COLS + 'background:#f1f5f9;border-top:1.5px solid rgba(148,163,184,0.28);margin-top:6px;'
            + 'grid-template-columns:1fr auto;">'
            + '<span style="font-size:0.78rem;font-weight:800;color:#1e293b;">Total desglose</span>'
            + '<span id="egresoBilletesTotal" style="font-size:0.95rem;font-weight:800;color:#94a3b8;text-align:right;">$0</span>'
            + '</div>'
            + '<div id="egresoBilletesAviso" style="display:none;padding:6px 12px 9px;font-size:0.72rem;font-weight:700;line-height:1.35;"></div>';

        cont.innerHTML = cabecera + EGR_GRUPOS.map(grupo).join('') + pie;
        _egrTotalBilletes();
    }

    // Suma el desglose, pinta el subtotal de cada fila y avisa si no cuadra con el monto
    window._egrTotalBilletes = function () {
        ...
        EGR_DENOMS.forEach(d => {
            ...
            total += n * d;
            const sub = document.getElementById('egrSub-' + d);
            if (sub) {
                sub.textContent = n ? _egrFmt(n * d) : '—';
                // Se reescribe el atributo style completo (no style.color) para que las reglas
                // de tema oscuro, que buscan "color:#1e293b" en el atributo, sigan aplicando.
                sub.setAttribute('style', n
                    ? 'font-size:0.82rem;font-weight:800;text-align:right;color:#1e293b;'
                    : 'font-size:0.82rem;font-weight:700;text-align:right;color:#94a3b8;opacity:0.7;');
            }
        });
        const lbl = document.getElementById('egresoBilletesTotal');
        const avi = document.getElementById('egresoBilletesAviso');
        if (!lbl) return total;
        const inpMonto = document.getElementById('egresoMonto');
        const monto = parseInt(((inpMonto && inpMonto.value) || '').replace(/\D/g, '')) || 0;

        if (!total) {
            lbl.setAttribute('style', 'font-size:0.95rem;font-weight:800;text-align:right;color:#94a3b8;');
            lbl.textContent = '$0';
            if (avi) { avi.style.display = 'none'; avi.textContent = ''; }
        } else if (monto && total !== monto) {
            lbl.setAttribute('style', 'font-size:0.95rem;font-weight:800;text-align:right;color:#b45309;');
            lbl.textContent = _egrFmt(total);
            if (avi) {
            ...
            lbl.setAttribute('style', 'font-size:0.95rem;font-weight:800;text-align:right;color:#166534;');
            lbl.textContent = _egrFmt(total);
            if (avi) {
                if (monto) {
                    avi.setAttribute('style', 'padding:6px 12px 9px;font-size:0.72rem;font-weight:700;line-height:1.35;color:#166534;');
                    avi.textContent = '✓ Coincide con el monto solicitado.';
                    avi.style.display = 'block';
