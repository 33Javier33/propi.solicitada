...
async function _sociosHandler(url, options) {
    ...
    switch (action) {
        ...
        // Historial completo de anticipos de un socio (activos + archivados)
        case 'getHistorialCompletoSocio': {
            ...
            const all = [
                ...(actRes.data  || []).map(a => ({ ...a, cantidad: a.monto, estado: 'ACTUAL' })),
                ...(histRes.data || []).map(a => ({ ...a, cantidad: a.monto }))
            ].sort((a, b) => String(b.fecha).localeCompare(String(a.fecha)));
            return _mockRes({ data: all });
        }
